...
--
-- This matches the schema created by `scripts/mariadb-init/001-schema.sql`, but:
-- - It does NOT include `CREATE DATABASE` / `USE` statements.
-- - It is safe to run even if the tables already exist (uses IF NOT EXISTS).
--
-- Recommended usage for an existing on-prem DB:
--   1) Ensure the DB already exists and the schema is created (via docker init)
--   2) Run: `npm run db:baseline`
--
-- Recommended usage for a brand-new DB (no docker init scripts used):
